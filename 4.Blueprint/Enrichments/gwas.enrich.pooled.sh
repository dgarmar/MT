...
# Only cufflinks and total rna

SQTLS_IN="$(bedtools intersect -a pooled.sqtls.5fdr.bed -b $GWAS_BED -c | awk '$5>0 {print $4"\t"$5}' | wc -l)"
NON_SQTLS_IN="$(bedtools intersect -a pooled.non-sqtls.5fdr.bed -b $GWAS_BED -c | awk '$5>0 {print $4"\t"$5}' | wc -l)"
...
echo $RATIO5  # Fold enrichment in GWAS-1Kb (%sQTLs/%non-sQTLs) at 1% FDR
