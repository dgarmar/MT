...
## Perform FDR correction and get significant sQTLs 
# 1%FDR 
sig.f = "sQTLs-sig1.tsv"
sqtls.df = suppressWarnings(sqtls(res.df, FDR=.01))
write.table(sqtls.df, file=sig.f,quote=FALSE, row.names=FALSE, col.names=TRUE, sep="\t")

# 5%FDR 
sig.f = "sQTLs-sig5.tsv"
sqtls.df = suppressWarnings(sqtls(res.df, FDR=.05))
write.table(sqtls.df, file=sig.f,quote=FALSE, row.names=FALSE, col.names=TRUE, sep="\t")
