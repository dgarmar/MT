...
##
## BatchJobs functions definition
##
...
makeClusterFunctionsAdaptive <- function (template.file){
    ...
    submitJob <- function(conf, reg, job.name, rscript, log.file,
        ...
        res <-  BatchJobs:::runOSCommandLinux("qsub", outfile, 
        ...
        if (grepl(max.jobs.msg, output, fixed = TRUE)) {
            ...
            makeSubmitJobResult(status = 0L, batch.job.id = str_extract(output,
                                                                        "\\d+"))
        }
    }
    ...
    makeClusterFunctions(name = "Adaptive", submitJob = submitJob,
                         killJob = killJob, listJobs = listJobs, 
                         getArrayEnvirName = getArrayEnvirName)
}
