...
if (!file.exists("sQTL")){
 cat("## WARNING: sQTL directory is not available 
      for this tissue. Any result can be reduced.")
 cat("\n")
 quit("no")
}
...
tmp = reduceResultsList(sQTL.reg, fun=function(job, res){
  write.table(res, file=res.f, quote=FALSE, row.names=FALSE, 
              col.names=!file.exists(res.f), append=file.exists(res.f), sep="\t")
})
