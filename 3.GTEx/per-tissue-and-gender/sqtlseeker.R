...
gene.bed.f = "/nfs/users/rg/dgarrido/run-GTEx/input/genes.protcod.bed"
...
colnames(tre.df)[3:ncol(tre.df)] = gsub(pattern="GTEX-([^-]+)-.*", 
...
submitJobs(sQTL.reg, 
           findNotDone(sQTL.reg), resources=list(walltime="6:0:0",
                                                 mem="4G", cores="1",queue=
                                                   "long-sl65,rg-el6,short-sl65"), 
