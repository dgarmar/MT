...
tissues.original <- sort(unique(sample.tissues[,3]))

# system("rm -rf results/*") # For debugging

## Iterate and run prepare.trans.exp.R for each tissue and both genders
...
for (tissue in tissues){
  ...
  for (g in 1:2){
    ...
    system(sprintf("Rscript ../../../bin/prepare.trans.exp.R %s %s 
                            2> stderr.txt > stdout.txt", t.index, g))
    setwd("..")
  }
  setwd("../..")
  t.index=t.index+1
